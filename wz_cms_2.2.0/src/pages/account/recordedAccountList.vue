...
<div>
    <el-form ref="form" :model="form" label-width="160px" :inline="true" align="left">
        <el-form-item label="流水单号">
            <el-input class="nn-input-s" v-model="form.withdrawOrderNo" placeholder="精确搜索"></el-input>
        </el-form-item>
        ...
        <div class="infoBox">
            <div class="infoCell">
                <p>【已入账，可提现】账户【流入】流水总额</p>
                <p class="color-red fs-24">+￥{{resultData.sumWithdrawAmountIn/100 | moneyFormat}}</p>
            </div>
            <p class="infoDivider"></p>
            <div class="infoCell">
                <p>【已入账，可提现】账户【流出】流水总额</p>
                <p class="color-green fs-24">-￥{{resultData.sumWithdrawAmountOut/100 |  moneyFormat}}</p>
            </div>
        </div>
        <div class="nn-form-line-h"></div>
        <div class="nn-table-wrapper">
            <el-table class="mt-20 nn-no-overflow-table" :data="tableData" style="width: 100%" align="left" border>
                <el-table-column label="流水单号" prop="withdrawOrderNo">
                </el-table-column>
                <el-table-column label="流水账号" prop="phone">
                </el-table-column>
                ...
                <el-table-column label="流水金额" prop="">
                    <template slot-scope="scope">
                       <p v-if="scope.row.transformAmount > 0 "  class="color-red" > ￥{{scope.row.transformAmount/100 | moneyFormat}}</p>
                       <p v-else-if="scope.row.transformAmount < 0" class="color-green"> ￥{{scope.row.transformAmount/100 | moneyFormat}}</p>
                       <p v-else>{{scope.row.transformAmount}}</p>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="审核时间" prop="">
                    <template slot-scope="scope">
...
export default {
    ...
    methods: {
        ...
        callTableData(paramJson) {
             ...
             sendPost(urls.getWithdrawTransformList, {data: JSON.stringify(paramJson)},(resultJson)=>{
                 this.tableData = resultJson.resultData.withdrawTransformList;
                 this.totalPageCount  =resultJson.resultData.withdrawTransformrCount;
