    ...
    <div class="orderDetail">
        ...
        <section class="vipBox van-hairline--top">
            <template v-if="orderState>1">
                ...
                <div class="downLoad">
                    <template v-if="!isApp">
                        <button class="downloadBtn" @click="onDownLoad">下载 超惠省app，使用会员特权</button>
                        <div class="notice">
                            <p>请提前下载app了解卡片邮寄情况；</p>
                            <p>收卡后打开app激活卡片，即可享受超值特权！</p>
                        </div>
                    </template>
                </div>
                <div class="downLoad">
                      <div class="notice">
                            <p >活动期间，再附赠您一张电子卡，</p>
                            <p >将通过短信&推送发给您，可直接用于激活。</p>
                      </div>
                      <button class="downloadBtn" @click="onUseCard">使用附赠电子卡，立即激活</button>
                      <button class="downloadBtn btnStyle2" @click="onToHome">先去购物返利</button>
                </div>
            </template>
            <template v-if="orderState == 1">
                <div class="failPay">
...
<script>
    ...
    export default {
        ...
        methods: {
            onUseCard(){
               this.$router.push('/card/activateCard');
            },
            onToHome(){
                if(isAndroidClient()){
                    APPTool.addJsCallback(this,'startNative',{
                        url:'fychs://org.vackapi.wool.home.HomeActivity?pos=0'
                    })
                }
                if(isIOSClient()){
                    APPTool.addJsCallback(this,'startNative',{
            ...
            appPayFn(){
                ...
                returnUrl = `/order/orderDetail?isOpenApp=true&isSimple=true&channel=${getUrlArgs('channel')}`;
                let params = {
                    cardOrderId : this.cardOrder.cardOrderId
                };
                this.request.post(this.urls.appPayCardOrderWithoutLogin,params,true).then(res=>{
                    let payData = {};
                    if(this.payWay == 5){
                        payData.config = res.resultData.config;
                        payData.payType = 1;
                    }else if(this.payWay == 4){
                        payData.prepayId = res.resultData.prepayId;
                        payData.payType = 3;
                    }
                    let sendData = {
                        payData,
        ...
        .vipBox{
            ...
            .downLoad{
                ...
                .btnStyle2 {
                    ...
                    border: 1px solid rgb(216, 177, 138);
                    background: rgb(255, 255, 255);
                    color: rgb(216, 177, 138);
                }
                .notice{
