...
// 获取hash的参数
global.getHashArgs = function(key){
...
global.addUrlParams= function (url,paramJson){
    paramJson.fychsChannel  = getUrlArgs('channel');
    let isHasParams = url.includes('?')
    let index = 0;
...
global.getUrlParam = function (paramName) {
    return decodeURIComponent((new RegExp('[?|&]'+paramName+'='+'([^&;]+?)(&|#|;|$)').exec(location.href)||[,""])[1].replace(/\+/g,'%20'))|| 'default'
}
...
global.STATS_SA = {
    ...
    trackLinkClick(e, clickName, clickDesc, param = {}) {
        ...
        sa.trackLink(e.target, 'elementClick', param);
    },
    quick(method,params = {}) {
...
global.STATS = {
    ...
    addAccess: function (paramJson) {
        ...
        //2：wap，3：iOS，4：android
        if (isAndroidClient()) {
            paramJson.platformType = 4;
        } else if (isIOSClient()) {
            paramJson.platformType = 3;
        } else {
            paramJson.platformType = 2;
        }
        paramJson.channelName = getUrlArgs('channel');
        Vue.prototype.request.post(Vue.prototype.urls.addAccess, paramJson,false);
    },
